...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--lr_schedule", type=str, default="onecycle",
                   choices=["none", "onecycle", "cosine_restart"],
                   help="onecycle: warmup (10%%) + cosine decay to lr/100. "
                        "cosine_restart: SGDR with T_0 and T_mult.")
    p.add_argument("--warmup_pct", type=float, default=0.1)
    p.add_argument("--cosine_t0", type=int, default=10000,
                   help="cosine_restart: iterations in first cycle.")
    p.add_argument("--cosine_tmult", type=int, default=2,
                   help="cosine_restart: cycle length multiplier.")
    p.add_argument("--cosine_eta_min", type=float, default=1e-5,
                   help="cosine_restart: minimum LR.")
...
def main() -> None:
    ...
    if args.lr_schedule == "onecycle":
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optim, T_0=args.cosine_t0, T_mult=args.cosine_tmult,
            eta_min=args.cosine_eta_min,
        )
        print(f"[overfit_sl] CosineAnnealingWarmRestarts: T_0={args.cosine_t0}, "
              f"T_mult={args.cosine_tmult}, eta_min={args.cosine_eta_min}")

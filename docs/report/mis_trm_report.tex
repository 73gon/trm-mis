...
\author{Malik Mardan\\ \normalsize Matriculation number 3834310 \quad Study programme: Master Computer Science}
\date{July 1, 2026}
...
solve hard reasoning tasks that usually call for much larger models.
...
\begin{itemize}\itemsep0.5pt
  \item \textbf{Iteration.} One gradient update on one mini-batch of graphs; ``$29$k
    iterations'' means $29{,}000$ updates. We evaluate on the validation set every
    $500$ iterations and keep the best checkpoint, so runs stop at different counts.
...
$n\in\{50,100,200,300\}$, each with a held-out validation split (disjoint seeds)
and exact Gurobi labels.
...
Every number below is measured on these held-out splits, never on training graphs
(for the benchmarks this is the official DIFUSCO test split). Because the same
split also drives early stopping and checkpoint selection, it serves as a
validation set, so the reported best values are mildly optimistic. The splits hold
$5000/5000/1000/300$ graphs for $n{=}50/100/200/300$ and $500$ graphs for each
benchmark.

\subsection{Scaling over problem size}

On all four small sizes the model learns cleanly (Fig.~\ref{fig:small}). The
validation BCE drops from its random-init value near $0.73$ to between $0.23$ and
$0.32$, while $\ppar$ climbs from about $0.886$ at initialisation to at least
$0.98$. Larger graphs reach their best checkpoint in fewer iterations because each
graph carries more signal, but they also start to overfit sooner since their
training pools are smaller (Table~\ref{tab:main}).

\begin{figure}[h]
  \centering
  \includegraphics[width=0.60\linewidth]{small_mis_curves.png}
  \caption{Held-out validation curves for the four small ER sizes. Left:
  validation BCE (lower is better). Right: approximation ratio $\ppar$
  (higher is better).}
  \label{fig:small}
\end{figure}
...
``best BCE'' is the lowest validation BCE over the run and ``best $\ppar$'' the
highest validation ratio; the two can occur at different checkpoints. Feasibility
is $1.00$ everywhere thanks to greedy decoding.}
\label{tab:main}
...
calibrated, so they reach both a low BCE and a high ratio. BCE therefore tracks
calibration against one labelling while $\ppar$ tracks set quality, and only the
latter reflects the objective we care about.
...
stopping on the validation score is therefore essential at this scale.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.60\linewidth]{satlib_n9000.png}
...
larger message-passing tensor. Whether a run fits depends on how many dense graphs
share one GPU batch, not on the pool size: the completed result on this distribution
used a larger pool ($25{,}000$ graphs) but only $8$ graphs per batch instead of
$16$, and reaches $\ppar=0.903$, well above a degree-heuristic baseline of $0.60$
but clearly the hardest case we tried.
...
repository at \url{https://github.com/73gon/trm-mis}.\par}

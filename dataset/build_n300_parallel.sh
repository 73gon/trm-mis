...
for i in 0 1 2 3 4 5; do
  ...
  .venv/bin/python -m dataset.build_mis_dataset_multilabel \
    ...
    --pool-size 16 --pool-gap 0.0 --pool-time-limit 120.0 --threads 1 \
    > "logs/build_n300_train_chunk_$i.log" 2>&1 &
  PIDS+=($!)
done

# Test chunks: 2 workers * 500 graphs = 1000 test.
for i in 0 1; do
  seed_start=$((8000000 + i * 100000))
  ...
  .venv/bin/python -m dataset.build_mis_dataset_multilabel \
    ...
    --d-min 7.35 --d-max 7.35 \
    --pool-size 16 --pool-gap 0.0 --pool-time-limit 120.0 --threads 1 \
    > "logs/build_n300_test_chunk_$i.log" 2>&1 &
  PIDS+=($!)
done

echo "Launched ${#PIDS[@]} workers: ${PIDS[*]}"
echo "${PIDS[*]}" > logs/build_n300_pids.txt
wait "${PIDS[@]}"
